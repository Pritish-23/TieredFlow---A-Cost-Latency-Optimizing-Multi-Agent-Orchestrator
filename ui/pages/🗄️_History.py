    ...
    with st.expander(
        ...
        for msg in messages:
            ...
            with st.chat_message("assistant"):
                ...
                cols = st.columns(5)
                cols[0].caption(f"🧠 **Task:** {msg.task_type.replace('TaskType.', '')}")
                ...
                cols[4].caption(f"{'🟢 Cache hit' if msg.served_from_cache else '🔵 Fresh call'}")

        # ── Delete session ─────────────────────────────────────────────────────

        st.divider()
        if st.button(f"🗑️ Delete Session {session.session_id}", key=f"del_{session.session_id}"):
